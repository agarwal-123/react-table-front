...
export default function CustomPaginationActionsTable() {
  ...
  const emptyRows =
    rowsPerPage - Math.min(rowsPerPage, rawData.length - page * rowsPerPage);
  ...
  function fun2() {
    if (fetchedData.length > 0)
      setRawData(
        fetchedData.slice(0, document.getElementById("standard-number").value)
      );
    else fun(document.getElementById("standard-number").value);
  }

  // componentDidMount() {
  //   fun().then((result) => {
  //     console.log(result, "jkj");
  //     setRawData(result);
  //   });
      ...
      <TableContainer component={Paper}>
        <Table className={classes.table} aria-label="custom pagination table">
          ...
          <TableBody>
            {(rowsPerPage > 0
              ...
              <TableRow key={row.word}>
                ...
                <TableCell style={{ width: 160 }}>{row.freq}</TableCell>
              </TableRow>
            ))}
          ...
          <TableFooter>
            <TableRow>
              <TablePagination
                rowsPerPageOptions={[10, 25, 40, { label: "All", value: -1 }]}
                colSpan={3}
